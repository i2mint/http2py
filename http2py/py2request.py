...
def mk_request_function(
    ...
    def request_func(*args, **kwargs):
        ...
        if "url_template" in method_spec:
            ...
            #  encoded_kwargs alone stopped working when I added a signature to
            #  Py2Request methods.
            #  But really, don't know what I'm doing, becaues this request_func
...
class Py2Request(object):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._method_specs = self._process_method_specs()
    ...
    def _process_method_specs(self):
        if self._dflt_method_func_from_method_spec == mk_request_function:  # TODO: WTF!
            def gen():
                for method_name, method_spec in self._method_specs.items():
    ...
    def _inject_method(
        ...
        if not callable(method_spec):
            method_spec, method_wrap = _mk_method_func_and_wrap(
                method_spec,
                method_func_from_method_spec or self._dflt_method_func_from_method_spec,
            )
...
def _difference_conserving_order(a, b):
    """
    Returns a list with the elements of a that are not in b, but in the same order as a.
    """
    return [x for x in a if x not in b]


def extract_format_names(string_template):
    """
    Extracts the names of the fields in a string template, returning a list of
    strings.

    >>> string_template = "Hello, {name}! You are {age} years old."
    >>> extract_format_names(string_template)
    ('name', 'age')
    """
    NAME_IDX = 1
    parsed_template = str_formatter.parse(string_template)
    return tuple(filter(bool, (x[NAME_IDX] for x in parsed_template)))
